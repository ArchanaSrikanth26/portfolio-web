...
import portrait from "@/assets/archana-portrait.jpg";
    ...
    <section id="top" className="relative min-h-screen flex items-center pt-28 sm:pt-32 pb-16 sm:pb-20 overflow-hidden">
      ...
      <div className="relative mx-auto max-w-6xl px-4 sm:px-6 w-full grid lg:grid-cols-[1.4fr_1fr] gap-12 items-center">
        <motion.div variants={container} initial="hidden" animate="show" className="max-w-4xl">
          <motion.div variants={item} className="inline-flex items-center gap-2 rounded-full border border-border glass px-3 py-1.5 text-xs font-mono text-muted-foreground mb-8">
          ...
          <motion.h1 variants={item} className="font-display font-bold leading-[0.9] text-4xl sm:text-6xl lg:text-8xl tracking-tighter">
            Archana
            <br />
            <span className="text-gradient">Srikanth.</span>
          </motion.h1>

          <motion.p variants={item} className="mt-8 max-w-2xl text-lg sm:text-xl text-muted-foreground leading-relaxed">
            Full Stack Developer crafting scalable web & mobile products with{" "}
            <span className="text-foreground">MERN</span>,{" "}
            <span className="text-foreground">Java Spring Boot</span>,{" "}
          ...
          <motion.div variants={item} className="mt-16 sm:mt-20 grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 max-w-xl">
            {[
              ...
              <div key={s.v} className="rounded-2xl border border-border/60 bg-background/40 p-4 sm:p-0 sm:border-0 sm:bg-transparent">
                <div className="font-display text-3xl sm:text-4xl font-bold text-gradient-accent">{s.k}</div>
        ...
        >
          ...
          <div className="relative rounded-[2rem] overflow-hidden border border-border glass shadow-glow">
            <img
              src={portrait}
